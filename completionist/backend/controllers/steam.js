...
const getSteamAchievements = authWrapper({
	authFunction: async (req, res, token) => {
		...
		// Check permissions
		$('h3').each((_, element) => {
			if ($(element).text().trim().includes('An error was encountered while processing your request')) {
        errorFound = true;
      }
		});
		if (errorFound) {
			return res.status(response_code.SUCCESS).json({ hasPermission: false, achievements: [], noOfLocked, token })
		}
		else {
			...
			if (achievements[achievements.length - 1]?.name?.includes('hidden achievements remaining')) {
				noOfLocked = parseInt(achievements[achievements.length - 1].name.split('hidden achievements remaining')[0]);
				achievements.pop();
			}
			return res.status(response_code.SUCCESS).json({ hasPermission: true, achievements, noOfLocked, token });
		}
	},
	onError: (res, error) => {
		console.error('Error fetching achievements:', error);

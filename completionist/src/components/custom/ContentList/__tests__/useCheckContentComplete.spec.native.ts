...
describe('useCheckContentComplete', () => {
	...
	it('checkContentComplete returns the correct content for incomplete', () => {
		const { result } = renderHook(() => useCheckContentComplete(), initialState);
		const data = result.current.checkContentComplete('c0c4db32-0e1a-4f9c-9707-6e510646237f');
	...
	it('checkContentComplete returns the correct content for complete', () => {
		...
		const { result } = renderHook(() => useCheckContentComplete(), updatedInitialState);
		const data = result.current.checkContentComplete('c0c4db32-0e1a-4f9c-9707-6e510646237f');
	...
	it('checkContentCompleteForCategory returns the correct content for complete', () => {
		...
		const { result } = renderHook(() => useCheckContentComplete(), updatedInitialState);
		const { result: itemsResult } = renderHook(() => useGetContent(), updatedInitialState);

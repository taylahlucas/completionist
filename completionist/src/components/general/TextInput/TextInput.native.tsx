...
const TextInput = React.forwardRef<RNTextInput, TextInputProps>(
  (props, ref) => {
    ...
    return (
      <TextInputContainer
        width={width ?? LARGE_WIDTH}
        height={height}
        style={containerStyle}
        multiline={multiline ?? false}>
        <Condition condition={!!leftComponent}>{leftComponent}</Condition>
        <RNTextInput
          testID={testID}
          ref={ref}
          style={{
            ...textInputStyle,
            ...defaultStyle['ListItemSubTitle'],
          ...
          ): void => onChangeText(event.nativeEvent.text ?? '')}
          {...props}
        ...
        <Condition
          condition={
            !!value && value?.length > 0 && inputStyle !== 'verification'
          }>
          <TextInputIcon
            testID={'reset-input'}
            onPress={onReset}

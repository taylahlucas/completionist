...
const useGetSettingsConfig = (): GetSettingsConfigReturnType => {
  ...
  const shouldShowCompletedItems = (): boolean => {
    return user.settings.configs.find(item => item.id === SettingsOptionEnum.COMPLETED_ITEMS)?.isActive ?? false;
  };

  const shouldShowDisabledSections = (): boolean => {
    return user.settings.configs.find(item => item.id === SettingsOptionEnum.DISABLED_SECTIONS)?.isActive ?? false;
  };

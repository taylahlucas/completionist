...
const useAuthEndpoints = (): AuthEndpointsReturnType => {
  ...
  const checkUserExists = async (email: string): Promise<CredentialsExistProps> => 
    await authInterceptor.post(`${url}/${checkUserExistsUrl}`, 
      {
  ...
  const signUp = async ({data}: SignUpProps): Promise<UserResponse> => 
    await authInterceptor.post(`${url}/${signupUrl}`, 
      {
      ...
      const signIn = async ({
        ...
        await authInterceptor.post(`${url}/${signinUrl}`, {
          email: email.toLocaleLowerCase(),
          pw,
          googleId,
        }).then((response) => {
      ...
      const linkAndSignIn = async ({
        ...
        await authInterceptor.patch(`${url}/${linkAndSignInUrl}`, 
          {
            email: email.toLocaleLowerCase(),
            pw,
            googleId,
